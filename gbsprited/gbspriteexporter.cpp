...
GBSpriteExporter::GBSpriteExporter(QSharedPointer<Sprite> sprite)
{
	m_sprite = sprite;
}

void GBSpriteExporter::run()
{
		...
		for( int ty = 0; ty < m_sprite->height() / 8; ty++ )
		{
			for( int y = 0; y < 8; y++ )
			{
				...
				for( int x = 0; x < 8; x++ )
				{
					...
					row_first  |= bit_first  << (x);
					row_second |= bit_second << (x);
				}
				notifyBytes( row_first, row_second );
			}
			notifyNewTile();
		}
	}
...
GBSpriteExporter::~GBSpriteExporter()
{
